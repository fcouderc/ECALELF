...
Categorize_region(){
./bin/ZFitter.exe -f data/validation/${file}.dat --regionsFile=data/regions/${region}.dat --addBranch=smearerCat invMass_var=${invMass_type} --saveRootMacro

#categorie
mv tmp/smearerCat_${region}_*${file}* friends/smearerCat/ 
for tag in `grep "^s" data/validation/${file}.dat | grep selected | awk -F" " ' { print $1 } '`
do
...
Test_1_job_highMass(){
./bin/ZFitter.exe -f data/validation/${file}.dat --regionsFile=data/regions/${region}.dat --invMass_var=${invMass_type} --commonCut=Et_35-noPF --autoBin --smearerFit --plotOnly --profileOnly --targetVariable=ptRatio*pt2Sum --targetVariable_min=0.5*0 --targetVariable_max=2*300 --targetVariable_binWidth=0.02*6 --configuration=random ${initParameters1}
#--corrEleType=EtaR9 --smearEleType=stochastic --initFile=init_RUN2.txt | tee withCorr.txt
}
