...
{
  //g has already its minimum at zero (in Y)
  //just declaration of ranges
  double rangeLimMax=0.;
  double rangeLimMin=0.;

  int Num = g->GetN(); double* Y = g->GetY(); int Locmin = TMath::LocMin(Num,Y); 
  double* X= g->GetX();
  double YMin = Y[Locmin];
  double XMin = X[Locmin];//x corresponding to YMin
  double YMax = YMin + 3;

  if(isScale){
    rangeLimMin= 0.96;
    rangeLimMax=1.04;
    cout<<"isScale min"<<rangeLimMin<<endl;
    cout<<"isScale max"<<rangeLimMax<<endl;
  }

  //Initial range for !(scale)
  if(!isScale){
    rangeLimMin=0.00;
    rangeLimMax=0.01;
    cout<<"not scale "<<rangeLimMax<<endl;
  }
  ...
  cout<<"[FIT STATUS]: Fit to pol2"<<endl;
  cout<<"[FIT STATUS]: range_min is "<<range_min<<endl;
  cout<<"[FIT STATUS]: range_max is "<<range_max<<endl;
  //TF1* fun = new TF1("fun",symmetricParabola,range_min,range_max,3);
  //fun->SetParameter(0,YMin);
  //fun->SetParameter(1,XMin);
  //fun->SetParLimits(2,0,1);
  g->Fit("pol2","0FR+","",range_min,range_max); //primo passo: ampio range
  //g->Fit(fun,"0FR+","",range_min,range_max);
  ...
  if(isScale){
    ...
    rangeLimMin= X[pos1];
    rangeLimMax= X[pos2];
    cout<<"isScale 2 min"<<rangeLimMin<<endl;
    cout<<"isScale 2  max"<<rangeLimMax<<endl;
  }
  ...
  if(!isScale){
    ...
    rangeLimMax= X[pos];
    //rangeLimMax=0.005;
    cout<<"not scale 2 "<<rangeLimMax<<endl;
  }
  ...
  //Initialization parameters for the asymParabola
  f2->SetParameter(0,0);
  f2->SetParameter(1,minX);
  //f2->FixParameter(1,(rangeLimMax + rangeLimMin)/2.);
  f2->SetParameter(2,1/(2*sigma*sigma));
  f2->SetParameter(3,1/(2*sigma*sigma));

  f2->SetParLimits(0,-100,100);
  double range=(rangeLimMax - rangeLimMin)/2.;
  f2->SetParLimits(1,minX -range,minX + range);
  f2->SetParLimits(2,0,1e10);
  f2->SetParLimits(3,0,1e10);
  g->Fit("f2","0FR+","",range_min,range_max); //secondo passo: range ristretto e minimo fissato

  TF1* f3 = new TF1("f3",asymmetricParabola,range_min,range_max,4);
  //Initialization parameters for the asymParabola
  f3->SetParameter(0,f2->GetParameter(0));
  f3->SetParameter(1,f2->GetParameter(1));
  f3->SetParameter(2,f2->GetParameter(2));
  f3->SetParameter(3,f2->GetParameter(3));

  g->Fit("f3","0LR","",range_min,range_max); //secondo passo: range ristretto e minimo fissato




  //g->Fit("f2","BR+","",range_min,range_max); //secondo passo: range ristretto
  /*
  ...
  */
  ...
  //TF1* fun2=new TF1("fun2",asymmetricParabola,0.9975,0.9995,4);                                                   
  //fun2->SetLineColor(kGreen);
  //double minX=fun->GetMinimumX();                                                                             
  //fun2->SetParameter(1,minX);                                                                                
  //double sigma=1./sqrt(2* fun->GetParameter(2));                                                              
  //fun2->SetParameter(2,1/(2*sigma*sigma));                                                                   
  //fun2->SetParameter(3,1/(2*sigma*sigma));                                                                   
  //g->Fit(fun2,"OFR+","",range_min,range_max);
  //TCanvas* cmy=new TCanvas();
  //g->Draw();
  //fun2->Draw("same");
  //cmy->SaveAs("tmp/gfitted0_insideIter.png");
  //if(g->GetEY()!=NULL){   
  //  for (int i=0;i<g->GetN();++i){                                                                                                                                             //
  //    *(g->GetY()+i) -= fun2->GetParameter(0);
  //  }                                                                                                                                                                          //
  //} 
  ...
  do{
    ...
    //TCanvas* cmy_2=new TCanvas();
    //g->Draw();
    //f2->Draw("same");
    //cmy_2->SaveAs("tmp/gfitted_insideIter.png");
    fun->GetRange(range_min, range_max);   
    ...
    //std::cout << "Iter: "<<iter<<" : Fit range: [ " << range_min << " : " << range_max << "]" << std::endl;
...
#endif
  }while ((fabs(chi2 - old_chi2)/old_chi2 > FIT_LIMIT) && (iter < ITER_MAX || iter < 4));//while (iter<1); 
  ...
  return f2; //this is asymm Parabola
  */
  //return fun;//this is the pol2
  //return fun2;//this is the asym Parabola
  return f2;//second step
...
void Plot(TCanvas *c, TGraphErrors *g, TF1 *fun, TPaveText *pt, bool isScale, bool isPhi, TString variable){
  cout<<"Calling Plot of macro_fit.C"<<endl;
  gStyle->SetOptTitle(0);  
  //float sigma=1./(sqrt(2* std::max(fun->GetParameter(2),fun->GetParameter(3)))); //asym Parabola
  float sigma=1./(sqrt(2*fun->GetParameter(2)));      
  cout<<"sigma is "<<sigma<<endl;
  c->Clear();
  // already done by rootlogon
  //c->SetTopMargin(0.15);
  ...
  if(isScale) {//scale
    ...
    xMin=std::min(fun->GetMinimumX()-2.5*sigma,xMin);//4
    xMax=std::max(fun->GetMinimumX()+2.5*sigma,xMax);//4
    xMin=std::max(xMin,0.96);
    xMax=std::min(xMax,1.04);
    yMin=TMath::MinElement(g->GetN(),g->GetY());
    g->GetXaxis()->SetRangeUser(xMin,xMax);//Settare la x in un grafico NON cambia i range in Y
    ...
    double xMin=std::max(fun->GetXmin()-2.5*sigma,0.);
    double xMax= std::min(fun->GetXmin()+2.5*sigma,0.15);
...
//       }
	...
	continue;
      ...
      if (!fun)
	{
	  std::cout << "FIT HAD BIG PROBLEMS!" << std::endl;
	  return;
	}
      
      if(fun->GetParameter(2)<=0 && fun->GetParameter(3)<=0) continue;
      cout<<"Plot in macro/macro_fit.C sees these variables"<<endl;
